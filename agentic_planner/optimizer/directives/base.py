...
class Directive(ABC):
    ...
    def apply(self, cfg: DJExecutableConfig) -> DirectiveResult:
        """
        Apply the directive without an index.

        This method builds an index internally. For batch processing,
        use apply_with_index() instead to avoid rebuilding the index.
        """
        from agentic_planner.optimizer.op_locator import ProcessIndex
        index = ProcessIndex.build(cfg.get("process", []))
        return self.apply_with_index(cfg, index)
